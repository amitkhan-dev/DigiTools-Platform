...
const Cart = ({carts,setCarts}) => {
  ...
  return (
    <div>
      ...
      {carts.map(item=> 
      <div key={item.id} className=' border border-gray-400 mx-3 p-8 mb-4 bg-[#f9fafc] rounded-lg'>
        <div>
          ...
          <div>
            <button  className=' btn text-[#ff3980] text-[16px] font-semibold' >Remove</button>
          </div>
        </div>
        </div>
      ...
      <button className='btn custom-gradient w-full rounded-full  text-[16px] font-bold text-[#ffffff] mb-3'>Proceed to Checkout</button>
      </div>
    </div>
  );
};
